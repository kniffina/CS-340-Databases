	...
	<div class="container filters">
		<div>
			...
			<?php
				...
				//delete the boss specified by the user input. If doesn't match anything will not return a value.
				if(!($stmt = $mysqli->prepare("DELETE FROM bosses WHERE name = ?"))){
					echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
				}

				if(!($stmt->bind_param("s",$_POST['BossName']))){
				...
				if(!$stmt->execute()){
				    ...
				    echo "Deleted " . $stmt->affected_rows . " row to bosses.";
				}
			?>
