...
<body>
	...
	<div class="container filters">
		<div>
			...
			//insert the lead_id and mem_id into the reports table. This will be characters that are
			//already created.
			if(!($stmt = $mysqli->prepare("INSERT INTO reports (lead_id, mem_id) VALUES (?,?)"))){
				echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
			}
			if(!($stmt->bind_param("ii",$_POST['LeaderName'], $_POST['MemberName']))){
			    echo "Bind failed: "  . $stmt->errno . " " . $stmt->error;
			}
			if(!$stmt->execute()){
			    echo "Execute failed: "  . $stmt->errno . " " . $stmt->error;
			} else {
			    echo "Added " . $stmt->affected_rows . " row to reports.";
			}
			?>
