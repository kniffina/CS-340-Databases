...
<body>
	...
	<div class="container filters">
		<div>
			...
			//insert mounts that user specified into database
			if(!($stmt = $mysqli->prepare("INSERT INTO mounts (name) VALUES (?)"))){
				echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
			}
			if(!($stmt->bind_param("s",$_POST['MountName']))){
			    echo "Bind failed: "  . $stmt->errno . " " . $stmt->error;
			}
			if(!$stmt->execute()){
			    echo "Execute failed: "  . $stmt->errno . " " . $stmt->error;
			} else {
			    echo "Added " . $stmt->affected_rows . " row to mounts.";
			}
