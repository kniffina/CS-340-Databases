...
<body>
	...
	<div class="container filters">
		<div>
			...
			//create a new zone and insert it into the zones table. This will
			//be a brand new value specified by the user.
			if(!($stmt = $mysqli->prepare("INSERT INTO zones (name) VALUES (?)"))){
				echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
			}
			if(!($stmt->bind_param("s",$_POST['ZoneName']))){
			    echo "Bind failed: "  . $stmt->errno . " " . $stmt->error;
			}
			if(!$stmt->execute()){
			    echo "Execute failed: "  . $stmt->errno . " " . $stmt->error;
			} else {
			    echo "Added " . $stmt->affected_rows . " row to zones.";
			}
