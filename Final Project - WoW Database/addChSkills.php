...
<body>
	...
	<div class="container filters">
		<div>
			...
			// put character skills into that character
			if(!($stmt = $mysqli->prepare("INSERT INTO chskills (char_id, skill_id) VALUES (?,?)"))){
				echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
			}
			if(!($stmt->bind_param("ii",$_POST['DisplayName'], $_POST['SkillName']))){
			    echo "Bind failed: "  . $stmt->errno . " " . $stmt->error;
			}
			if(!$stmt->execute()){
			    echo "Execute failed: "  . $stmt->errno . " " . $stmt->error;
			} else {
			    echo "Added " . $stmt->affected_rows . " row to characters skills.";
			}
			?>
