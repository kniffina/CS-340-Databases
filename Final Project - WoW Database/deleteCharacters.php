	...
	<div class="container filters">
		<div>
			...
			<?php
				...
				//delete a character from the character table if it is found by the user input specified.
				if(!($stmt = $mysqli->prepare("DELETE FROM characters WHERE dname = ?"))){
					echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
				}

				if(!($stmt->bind_param("s",$_POST['DisplayName']))){
				...
				if(!$stmt->execute()){
				    ...
				    echo "Deleted " . $stmt->affected_rows . " row to characters.";
				}
